      ...
      <div className='grid grid-cols md:grid-cols lg:grid-cols-3 gap-6 mt-12'>
        {exclusiveOffers.map((items)=>(
          <div key={items._id} className='group relative flex flex-col items-start justify-between gap-1 pt:12 md:pt-18 px-4 rounded-xl text-white bg-no-repeat bg-cover bg-center' style={{backgroundImage:`url(${items.image})`}}>
            ...
            <p className='text-xs text-white/70 mt-3'>Expires {items.description}</p>
            </div>
            <button className='flex items-center gap-2 font-medium cursor-pointer mt-4 mb-5'>
              View Offers
              <img className='invert group-hover:translate-x-1 trainsition-all' src={assets.arrowIcon} alt="arrow-icon"/>
            </button>
            </div>
        ))}
      </div>
